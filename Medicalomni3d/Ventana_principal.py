...
from Medicalomni3d.Configuracion_medicalomni3d import Configuracion_ventana, Estilos, Leer_imagenes, resource_path
...
class Ventana_Principal_MedicalOmni3D(tk.Toplevel):
    def __init__(self, master=None, usuario=None):
        ...
        self.espaciado = tk.BooleanVar(
            value=self.configuracion_sistema["modelos"][name_modelo]["Espaciado"] if self.configuracion_sistema[
                                                                                         "modelo_seleccionado"] != "" else True)
        ...
        self.botones_menu = [{"nombre": "Banco de imágenes", "imagen": self.imagen_tabla, "y": 360,
                              "comand": self.Frames_banco_imagenes},
                             {"nombre": "Configuración de Modelos", "imagen": self.imagen_configurar, "y": 420,
                              "comand": self.Pagina_configuracion},
                             {"nombre": "Salir", "imagen": self.imagen_salir, "y": 480,
                              "comand": self.Confirmar_salida}]
    ...
    def Iconos_menu_deslizable(self):
        self.icono_app = resource_path("medicalomni3d.ico")
        self.imagen_salir = Leer_imagenes(resource_path("exit.png"), (30, 30))
        self.imagen_configurar = Leer_imagenes(resource_path("settings.png"), (30, 30))
        self.imagen_tabla = Leer_imagenes(resource_path("table_view.png"), (30, 30))
        self.imagen_user = Leer_imagenes(resource_path("account_circle.png"), (30, 30))
        self.imagen_import = Leer_imagenes(resource_path("image_arrow_up.png"))
        self.imagen_export = Leer_imagenes(resource_path("file_save.png"))
        self.imagen_cancelar = Leer_imagenes(resource_path("close.png"), (30, 30))
    ...
    def Frames_banco_imagenes(self):
        ...
        self.frame_configuracion_tabla = ttk.Frame(self.frame_page_banco_imagenes,
                                                   style="Page_configuracion_banco.TFrame")
    ...
    def Cargar_imagenes(self):
        try:
            ...
            if lista:
                ...
                self.imagenes_codificadas = Configuracionnnunetv2.Codificacion_imagenes(
                    lista_imagenes_selecionadas=lista_codificar)
                lista.clear()
    ...
    def Eliminar_imagen(self, evento):
        try:
            ...
            if len(self.lista_imagenes_tabla) > 0:
                ...
                if respuesta:
                    Configuracionnnunetv2.Eliminarcion(lista_imagenes_selecionadas=self.lista_imagenes_tabla,
                                                       diccionario_paths=self.imagenes_codificadas)
                    ...
                    messagebox.showinfo(title="Eliminar imágenes",
                                        message=f"Se eliminaron correctamente las siguientes imágenes:\n{text}")
                    self.Cargar_imagenes()
            else:
        ...
        try:
            ...
            if lista_imagenes:
                if modelo_seleccionado != "Importar modelo":
                    ...
                    respuesta = messagebox.askyesno(title="Ejecutando Inferencia",
                                                    message=f"Desea ejecutar la inferencia de la imagen:\n{text}")
                    if respuesta:
                        self.imagenes_codificadas = Configuracionnnunetv2.Codificacion_imagenes(
    ...
    def Frames_banco_botones(self):
        ...
        self.boton_importar = ttk.Button(self.frame_botones, text="Importar Imagen", image=self.imagen_import,
                                         compound=tk.LEFT, cursor="hand2",
                                         command=lambda: self.Almacenamiento_imagenes(evento=None))
        self.boton_export = ttk.Button(self.frame_botones, text="Exportar Imagen", image=self.imagen_export,
                                       compound=tk.LEFT, cursor="hand2",
                                       command=lambda: self.exportar_imagen(evento=None))
        self.boton_eliminar = ttk.Button(self.frame_botones, text="Eliminar Imagen", image=self.imagen_eliminar,
                                         compound=tk.LEFT, cursor="hand2",
                                         command=lambda: self.Eliminar_imagen(evento=None))
        self.boton_selecion_modelo = ttk.Combobox(self.frame_botones, values=self.eleccion_modelos, state="readonly",
                                                  width=22, font=(" ", 9), justify="center", height=5, cursor="hand2")
        self.boton_inferencia = ttk.Button(self.frame_botones, text="Ejecutar modelo",
                                           image=self.imagen_ejecutar_modelo, compound=tk.LEFT, cursor="hand2",
                                           command=lambda: self.Ejecutar_modelo(evento=None))
    ...
    def exportar_imagen(self, evento):
        try:
            ...
            ruta = filedialog.askdirectory(title="Seleccione la carpeta para exportar las mascaras",
            ...
            if ruta:
                ...
                for item in self.tabla_banco.selection():
                    ...
                    self.lista_imagenes_tabla.append(imagen)
                if len(self.lista_imagenes_tabla) > 0:
                    Configuracionnnunetv2.Exportacion_imagenes(tipo_extension=self.variable_extension.get(),
                                                               lista_imagenes_selecionadas=self.lista_imagenes_tabla,
                                                               path_exportacion=ruta,
                                                               diccionario_paths=self.imagenes_codificadas)
                    for imagen in self.lista_imagenes_tabla:
    ...
    def importar_modelo(self, event):
        try:
            ...
            ruta = filedialog.askopenfile(title="Seleccione el modelo para importar", initialdir=initialdir,
                                          filetypes=[("Archivos ZIP", "*.zip")])
            if ruta:
                Configuracionnnunetv2.ventana_importar_modelo(master=self, path_model=ruta.name,
                                                              callback=self.actualizar_modelos)
    ...
    def guardar_configuracion_sistema(self, event):
        respuesta = messagebox.askyesno(title="Guardar configuración del sistema",
                                        message="¿Desea guardar los cambios realizados en la configuración del sistema?")
        if respuesta:
            if self.boton_modelos_configuracion.get() != "Importar modelo":
                self.configuracion_sistema["modelos"][self.boton_modelos_configuracion.get()][
                    "Normalizacion"] = self.normalizacion.get()
                self.configuracion_sistema["modelos"][self.boton_modelos_configuracion.get()][
                    "Espaciado"] = self.espaciado.get()
                self.configuracion_sistema["modelos"][self.boton_modelos_configuracion.get()][
                    "device"] = self.boton_dispositivo_configuracion.get()
                self.configuracion_sistema["modelos"][self.boton_modelos_configuracion.get()]["nuevo_espaciado"] = [
                    int(self.entry_x.get()), int(self.entry_y.get()), int(self.entry_z.get())]
                ...
                DAOMedicalOmni3D.Insertar_registro_intento(self.usuario, texto)
                messagebox.showinfo(title="Configuración guardada",
                                    message="Los cambios en la configuración del sistema se han guardado correctamente.")
                with open(self.archivojson, "w") as f:
                    json.dump(self.configuracion_sistema, f, indent=4)
                self.configuracion_sistema = Configuracionnnunetv2.Configuracion_apcivmapcas_json()
            else:
                messagebox.showerror(title="Error al guardar la configuración",
                                     message="No se guardaron los cambios. Importe un modelo válido.")
        self.frame_immportacion_modelo.focus()
    ...
    def frame_importacion_modelo(self):
        ...
        ttk.Label(self.frame_immportacion_modelo, style="Banner.TLabel", text="Configuración de Modelos",
                  font=(" ", 14, "bold")).pack(anchor="w", padx=15, pady=10)
        ...
        ttk.Label(frame_modelos, text="Modelo seleccionado:", style="Config.TLabel").grid(row=0, column=0, padx=10,
                                                                                          pady=10)
        ttk.Label(frame_modelos, text="Dispositivo de ejecución:", style="Config.TLabel").grid(row=0, column=2, padx=10,
                                                                                               pady=10)
        self.boton_modelos_configuracion = ttk.Combobox(frame_modelos, values=self.eleccion_modelos, state="readonly",
                                                        width=25, cursor="hand2")
        self.boton_dispositivo_configuracion = ttk.Combobox(frame_modelos, values=self.dispositivo, state="readonly",
        ...
        boton_importar_modelo = ttk.Button(frame_modelos, text="Importar Modelo", image=self.imagen_update_modelo,
                                           cursor="hand2", command=lambda: self.importar_modelo(event=None),
                                           compound=tk.LEFT)
        boton_importar_modelo.bind("<Return>", self.importar_modelo)
        boton_eliminar = ttk.Button(frame_modelos, text="Eliminar Modelo", cursor="hand2",
                                    command=lambda: self.Eliminar_modelo(event=None))
        boton_eliminar.bind("<Return>", self.Eliminar_modelo)
        self.boton_normalizacion = ttk.Checkbutton(frame_pre, text="Aplicar normalización", variable=self.normalizacion,
                                                   cursor="hand2")
        self.boton_espaciado = ttk.Checkbutton(frame_pre, text="Aplicar cambio de espaciado", variable=self.espaciado,
                                               cursor="hand2")
        ...
        boton_guardar = ttk.Button(self.frame_immportacion_modelo, text="Guardar configuración",
                                   command=lambda: self.guardar_configuracion_sistema(event=None))
        ...
        if self.boton_modelos_configuracion.get() != "Importar modelo":
            ...
            if respuesta:
                ...
                if self.eleccion_modelos:
                    self.boton_modelos_configuracion.config(values=self.eleccion_modelos)
                    self.boton_modelos_configuracion.current(0)
                    self.configuracion_sistema['modelo_seleccionado']=self.boton_modelos_configuracion.get()
                    self.cargar_configuracion(even=None)
                else:
                    self.boton_selecion_modelo.configure(values=self.eleccion_modelos)
                    self.boton_selecion_modelo.set("Importar modelo")
                    self.boton_modelos_configuracion.set("Importar modelo")
                    self.configuracion_sistema['modelo_seleccionado'] =""
    ...
    def Cargar_datos_usuarios(self):
        if self.lista_usuarios is None:
            ...
            for usuario in self.lista_usuarios:
                self.tabla_usuarios.insert(parent="", index=tk.END,
                                           values=(usuario.id_usuario, usuario.username, usuario.rol,
                                                   "Bloqueado" if usuario.bloqueado != 0 else "Desbloqueado",
                                                   usuario.fecha))
    ...
    def frame_tabla_usuarios(self):
        ...
        self.tabla_usuarios = ttk.Treeview(self.frame_tabla_usuario, columns=self.columna_tabla_usuario,
        ...
        scroll = ttk.Scrollbar(self.frame_tabla_usuario, orient=tk.VERTICAL, command=self.tabla_usuarios.yview,
                               cursor="arrow")
        ...
        ttk.Separator(self.frame_gestion_usuarios, orient="horizontal").grid(row=2, column=0, columnspan=2,
                                                                             sticky=tk.EW, pady=25)
        ...
        if lista_usuarios:
            ...
            if not (contador == 1 and self.tabla_usuarios.item(self.tabla_usuarios.selection()[0])['values'][
                ...
                respuesta = messagebox.askyesno(title="Eliminar usuario",
                                                message=f"¿Está seguro de que desea eliminar el siguiente usuario?\n\nUsuario: {usuario.username}")
                if respuesta:
                ...
                messagebox.showerror(title="Eliminar usuario",
                                     message="No es posible eliminar el último administrador del sistema.")
        else:
    ...
    def gestion_usuarios(self):
        ...
        ttk.Label(self.tab_usuarios, style="Menu_deslizable.TLabel", text="Gestión de usuarios",
                  font=(" ", 20, "bold")).grid(row=0, column=0, padx=20, pady=5, sticky=tk.W)
        ...
        tk.Label(self.frame_buscador, text="Buscar:", bg="#071640", foreground="white", font=(" ", 12, " ")).grid(row=0,
                                                                                                                  column=0,
                                                                                                                  sticky=tk.W,
                                                                                                                  pady=5)
        self.buscador = ttk.Entry(self.frame_buscador, width=30, font=(" ", 12, " "))
        self.boton_buscador = ttk.Button(self.frame_buscador, image=self.imagen_buscar_bd, cursor="hand2",
                                         command=lambda: self.Buscar_usuarios(evento=None))
    ...
    def Mostrar_ocultar_password(self, event):
        if self.mostrar_crear:
            ...
            self.imagen_icono_monstrar = Leer_imagenes(resource_path("visibility_off.png"), (12, 13))
        ...
        if re.match(self.email_pattern, self.correo_editar_usuario.get().lower()):
            ...
            if self.cambiar_password.get():
                if re.match(self.password_pattern, self.contrasena_editar_usuario.get()):
                    ...
                    messagebox.showerror(title="Contraseña inválida",
                                         message="La contraseña debe tener al menos 8 caracteres, una letra mayúscula y un número o carácter especial.")
            else:
                self.usuario_editar.username = self.correo_editar_usuario.get()
                self.usuario_editar.rol = self.Rol_editar_usuario.get()
                self.usuario_editar.bloqueado = self.Estado_editar_usuario.get()
    ...
    def Editar_usuario(self, event):
        if self.tabla_usuarios.selection():
            ...
            self.usuario_editar = Usuario(id_usuario=lista_usuario[0], username=lista_usuario[1], rol=lista_usuario[2],
                                          bloqueado=lista_usuario[3], fecha=lista_usuario[4])
    ...
    def Guardar_usuario(self, event):
        if re.match(self.email_pattern, self.correo_nuevo_usuario.get().lower()):
            if re.match(self.password_pattern, self.contrasena_nuevo_usuario.get()):
                usuario_nuevo = Usuario(username=self.correo_nuevo_usuario.get().lower(),
                                        password=self.contrasena_nuevo_usuario.get(), rol=self.Rol_nuevo_usuario.get(),
                                        bloqueado=self.Estado_nuevo_usuario.get())
                respuesta = messagebox.askyesno(
                    title="Nuevo usuario",
                    message=(
    ...
    def Ventana_editar_usuario(self):
        ...
        ttk.Label(frame_principal, text="Editar usuario", font=(" ", 16, "bold")).grid(row=0, column=0, columnspan=2,
                                                                                       pady=(0, 25))
    ...
    def Ventana_creacion_usuario(self, event):
        ...
        frame_principal.columnconfigure(1, weight=1)
        frame_principal.pack(fill="both", expand=True)
        ttk.Label(frame_principal, text="Crear nuevo usuario", font=(" ", 16, "bold")).grid(row=0, column=0,
                                                                                            columnspan=2, pady=(0, 25))
        ttk.Label(frame_principal, text="Correo:").grid(row=1, column=0, sticky=tk.E, pady=10)
    ...
    def frame_tabla_registros(self):
        ...
        self.tabla_logs = ttk.Treeview(self.frame_tabla_logs, columns=self.columna_tabla_logs, show="headings",
                                       cursor="hand2")
        ...
        scroll_v = ttk.Scrollbar(self.frame_tabla_logs, orient=tk.VERTICAL, command=self.tabla_logs.yview,
                                 cursor="arrow")
        scroll_h = ttk.Scrollbar(self.frame_tabla_logs, orient=tk.HORIZONTAL, command=self.tabla_logs.xview,
                                 cursor="arrow")
    ...
    def Cargar_datos_logs(self, usuario: str = None, accion: str = None):
        if self.lista_logs is None:
            ...
            for registro in self.lista_logs:
                self.tabla_logs.insert(parent="", index=tk.END,
                                       values=(registro.fecha, registro.username, registro.accion))
    ...
    def gestion_log(self):
        ...
        ttk.Label(self.tab_logs, style="Menu_deslizable.TLabel", text="Gestión de Registros",
                  font=(" ", 20, "bold")).grid(row=0, column=0, padx=20, pady=5, sticky=tk.W)
        ...
        tk.Label(self.frame_filtro_logs, bg="#071640", foreground="white", text="Usuarios:").grid(row=0, column=0,
                                                                                                  padx=10, pady=20)
        self.combox_usuarios = ttk.Combobox(self.frame_filtro_logs, values=self.list_logs_usuarios, width=25, height=3,
                                            state="readonly", cursor="hand2")
        tk.Label(self.frame_filtro_logs, bg="#071640", foreground="white", text="Acciones:").grid(row=0, column=2,
                                                                                                  padx=5, pady=20)
        self.combox_acciones = ttk.Combobox(self.frame_filtro_logs, values=self.logs_accion, height=3, state="readonly",
                                            cursor="hand2")
        self.combox_acciones.current(0)
        self.boton_filtro = ttk.Button(self.frame_filtro_logs, text="Filtrar", cursor="hand2",
                                       command=lambda: self.Filtrar_logs(event=None))
        self.boton_limpiar = ttk.Button(self.frame_filtro_logs, text="Limpiar", cursor="hand2",
                                        command=lambda: self.limpiar_tabla_registro(event=None))
    ...
    def Logo_banner(self):
        ...
        self.label_username = ttk.Label(self.frame_banner, style="Banner.TLabel", image=self.imagen_user,
                                        text=self.usuario.username, compound=tk.LEFT)
    ...
    def Creacion_menu_deslizable(self):
        ...
        for boton_info in self.botones_menu:
            boton = ttk.Button(self.frame_menu, image=boton_info["imagen"], style="Menu_deslizable.TButton",
                               cursor="hand2")
            ...
            label = ttk.Label(self.frame_menu, text=boton_info["nombre"], style="Menu_deslizable.TLabel",
                              cursor="hand2")
            label.place(x=80, y=boton_info["y"] + 10, width=160)
            boton_info["widget"] = [boton, indicador, label]
            label.bind("<Button-1>",
                       lambda event, ind=indicador, funcion=boton_info["comand"]: self.Mostrar_ocultar_indicador(
                           indicador_2=ind, page=funcion, animar=True))
            boton.configure(command=lambda ind=indicador, funcion=boton_info["comand"]: self.Mostrar_ocultar_indicador(
                indicador_2=ind, page=funcion, animar=True))
        for boton_info in self.botones_menu:
...
if __name__ == "__main__":
    usuario = Usuario(id_usuario=4, username="andresf@gmial.com", password="12345", rol="admin", intentos_fallidos=0,
                      bloqueado=0)
    user = Usuario(id_usuario=1, username="felipe14@gmail.com", password="12345", rol="Administrador",
